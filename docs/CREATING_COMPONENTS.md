...
This guide explains how to create new components in geoLARP following our enforced component structure standards.
...
docker compose exec geolarp pnpm run audit:components

# Validate for CI/CD (exits with error if non-compliant)
docker compose exec geolarp pnpm run validate:structure
```
...
docker compose exec geolarp pnpm run migrate:components
```
...
```bash
docker compose exec geolarp pnpm run audit:components -- --verbose
```
