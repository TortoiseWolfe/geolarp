...
2. Use `tmux send-keys -t geolarp:N 'command' Enter` to push commands
...
```json
{
  "generator-1": "geolarp:7",
  "generator-2": "geolarp:8",
  "generator-3": "geolarp:9",
  "planner": "geolarp:6",
  "validator": "geolarp:13"
}
```
...
```bash
tmux capture-pane -t geolarp:7 -p | tail -1 | grep -q "bypass permissions"
```
