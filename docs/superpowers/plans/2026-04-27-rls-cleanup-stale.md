...
**Goal:** Add a Vitest `globalSetup` hook to `pnpm test:rls` that scrubs orphan `*@geolarp.test` users + their FK-blocking dependent rows before tests collect, so killed prior runs don't wedge the suite.
...
```bash
docker compose exec geolarp git checkout -b 050/rls-cleanup-stale
```
...
```bash
docker compose exec geolarp pnpm vitest run tests/unit/rls-cleanup.test.ts
```
...
```bash
docker compose exec geolarp git add tests/unit/rls-cleanup.test.ts tests/rls/__setup__/cleanup-stale-impl.ts
docker compose exec geolarp git commit -m "feat(rls): cleanup-stale impl + 4 regression cases
...
for every *@geolarp.test user. Best-effort: errors are logged
...
- No-op when no geolarp.test users exist.
...
```bash
docker compose exec geolarp git add tests/rls/__setup__/cleanup-stale.ts
docker compose exec geolarp git commit -m "feat(rls): globalSetup wrapper for cleanup-stale
...
```bash
docker compose exec geolarp pnpm test:rls 2>&1 | tail -30
```
...
```bash
docker compose exec geolarp git add vitest.rls.config.ts
docker compose exec geolarp git commit -m "feat(rls): wire cleanup-stale globalSetup into pnpm test:rls

Killed prior runs that left orphan payment_intents/subscriptions/profiles
referencing *@geolarp.test auth users would wedge createTestUser
...
VALUES ('00000000-0000-4000-8000-000000050050', 'test-user-a@geolarp.test', 'fake');

INSERT INTO payment_intents (template_user_id, amount, currency, type, customer_email)
VALUES ('00000000-0000-4000-8000-000000050050', 1000, 'usd', 'one_time', 'test-user-a@geolarp.test');
```

- [ ] **Step 3: Run pnpm test:rls and observe**

```bash
docker compose exec geolarp pnpm test:rls 2>&1 | head -10
```
...
Adds a Vitest \`globalSetup\` to \`pnpm test:rls\` that scrubs orphan \`*@geolarp.test\` users and their FK-blocking dependent rows (\`payment_intents\`, \`subscriptions\`, \`user_profiles\`) before any RLS test file collects. The fixture's \`createTestUser\` retry path remains as the second line of defense; this hook just prevents the FK-block that defeated it on 2026-04-26 (and required manual psql cleanup).
...
```bash
docker compose exec geolarp git checkout main
git fetch --prune origin
git merge --ff-only origin/main
...
- Production-safety filter (`@geolarp.test` only): Task 3 step 1 + Task 2 test 2 ✓
