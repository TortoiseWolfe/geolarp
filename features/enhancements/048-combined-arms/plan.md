...
**Project Type**: Web client + **separate deployable game server** — the first geoLARP feature with a non-Pages runtime component.
...
| IV. Docker-First Development                     | ✅         | Match server, Redis, and LiveKit run as compose services under a `game` profile (mirroring the `supabase` profile). All installs/tests via `docker compose exec geolarp ...`; server dev via `docker compose --profile game up`.                              |
