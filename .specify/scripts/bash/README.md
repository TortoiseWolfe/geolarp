...
| `config.sh`                  | geoLARP-specific | sourced by `common.sh` (top of file)                               |
...
| `update-agent-context.sh`    | geoLARP stub     | `/speckit.plan` Phase 1 step 3                                     |

Upstream: <https://github.com/github/spec-kit/tree/main/scripts/bash>

## geoLARP adaptations

Upstream SpecKit assumes flat `specs/<NNN-name>/` layout. geoLARP uses
...
`update-agent-context.sh` is a deliberate no-op: geoLARP's `CLAUDE.md` is
...
Then re-apply the geoLARP adaptations listed above. The patches are small
(~50 lines total) and the diffs in git history show what to re-apply.

The `config.sh` and `update-agent-context.sh` files are geoLARP-only — never
