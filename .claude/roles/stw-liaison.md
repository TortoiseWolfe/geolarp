...
Manage the `stw` tmux session while coordinating with the main geoLARP
...
17 wireframes exist in SpokeToWork-MVP but need quality review against geoLARP standards.
...
| `stw/.terminal-status.json`                    | StW queue (separate from geoLARP) |

## Authority

| Can Do                        | Cannot Do                      |
| ----------------------------- | ------------------------------ |
| Create `stw` tmux session     | Modify geoLARP terminals  |
...
The Main Operator focuses on geoLARP internal work.
