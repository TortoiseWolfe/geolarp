...
| ---------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `public/manifest.json` | yes     | Reviewable: a build configured for the wrong root rewrites `start_url`, `scope` and every icon path, and committing that breaks PWA install. `scripts/__tests__/generated-manifest.test.js` fails on it, and derives the expected base path the same way `detect-project.js` does — **a `public/CNAME` means the apex, so this repo's own value is `/`, not `/geolarp/`** (#95). Do NOT "fix" a local build's output back to `/geolarp/`: that was the stale value, it is what this repo can never generate, and reverting it is how four sessions lost the correct file. |
